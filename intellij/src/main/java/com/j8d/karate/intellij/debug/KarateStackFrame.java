...
public class KarateStackFrame extends XStackFrame {
    ...
    public KarateStackFrame(KarateDebugProcess debugProcess, JsonObject frame) {
        ...
        // Resolve source position - this is safe because KarateStackFrame is created on a
        // background thread (from CompletableFuture.thenAccept), not on EDT.
        XSourcePosition resolvedPosition = null;
        if (sourcePath != null) {
            VirtualFile file = resolveSourceFile(sourcePath);
            if (file != null) {
                // DAP uses 1-based lines, IntelliJ uses 0-based
                resolvedPosition = XDebuggerUtil.getInstance().createPosition(file, line - 1);
            }
        }
        this.sourcePosition = resolvedPosition;
    }
    ...
    private VirtualFile findClassSourceFile(String className) {
        ...
        // Skip PSI lookup on EDT to avoid SlowOperations violations.
        // The PSI lookup (especially getNavigationElement) can trigger file indexing
        // and stub tree building which is prohibited on the EDT in IntelliJ 2025.1+.
        // For now, return null and rely on source references for library classes.
        if (ApplicationManager.getApplication().isDispatchThread()) {
            LOG.debug("Skipping PSI lookup for " + className + " (on EDT)");
            return null;
        }
